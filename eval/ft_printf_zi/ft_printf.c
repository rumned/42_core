...
/*   Updated: 2023/11/12 16:59:02 by zyeoh            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_print_space(t_pflags *flags)
{
	...
	flags->out_len--;
	while (++flags->out_len + flags->diff_len < flags->w_len && !flags->zero
		&& !flags->minus)
		write(STDOUT_FILENO, " ", 1);
	if (flags->plus == 1)
	{
	...
	flags->out_len--;
	while (++flags->out_len < flags->w_len && flags->minus == 2)
		write(STDOUT_FILENO, " ", 1);
	flags->minus++;
}
...
{
	while ((padding < flags->w_len && flags->zero) || padding < flags->pre_len)
	{
